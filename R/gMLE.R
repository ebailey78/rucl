#' Maximum Likelihood Estimates of gamma Distribution Parameters
#' 
#' Calculates MLE of shape, scale, and rate parameters for the provided dataset 
#' assuming a gamma distribution.
#' 
#' @param x numeric vector containing dataset
#' @param precision indicates to how many decimal places to estimate values
#' @param bias.correct \code{logical} indicating whether bias correction should 
#'   be performed
#' @param neg indicates how function should behave if it encounters negative
#'   values in \code{x}
#' @param \dots additional values to pass to/from other functions
#' @return list containing shape, scale, and rate estimates
#' @author Eric Bailey
#' @examples
#' gmle(rgamma(100, shape = 2, scale = 3))

gmle <- function(x, precision = 1e-5, bias.correct = TRUE, na.rm = FALSE, 
                 neg = "small", ...) {
  
  if(na.rm) {
    x <- x[!is.na(x)]
  } else if(any(is.na(x))) {
    return(list(shape=NA, scale=NA, rate=NA))
  }
  
  # Deals with datasets with 0 or negative values
  if(min(x) <= 0) {
    
    if(neg == "remove") {
      x <- x[x > 0]
    } else if(neg == "small") {
      x[x<=0] <- .Machine$double.eps
    } else if (is.na(neg)) {
      return(list(shape = NA, scale = NA, rate = NA))
    } else {
      stop("gamma distributed data cannot contain negative numbers")
    } 
    
  }
  
  n <- length(x)
  m <- sum(x)/n
  y <- log(x)
  
  M <- log(m) - sum(y) / n  # As described on page 47 of Tech Guide
  k = 0                                   # Arbitrary starting point for k
  k1 <- 1/(4*M)*(1 + sqrt(1 + (4/3)*M))   # Starting estimate of k
  c <- 0; stopper <- 100                  # Counter to make sure no inf loops
  
  #Iterative estimatation of k
  while(abs(k-k1) > precision & c < stopper) {
    k <- k1 * ((log(k1) - digamma(k1)) / M)
    k1 = k
    c = c + 1
  }
  
  shape = k
  
  if(c == stopper) {
    shape <- NA
  } else {
    if(bias.correct) shape <- (n - 3) * k / n + 2 / (3 * n) # Bias correction (Eq. 2-30)
  }
  
  scale = sum(x)/length(x)/shape
  
  list(shape = shape, scale = scale, rate = 1/scale)
  
}